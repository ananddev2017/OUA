...
public class IGP_TC_198 extends KeywordUtil{
	String stepInfo="";
	...
	@Test(
			testName="IGP_TC_198",
			groups={"Payment"}, 
			description="Payment Page-Credit Card & Debit card: Ensure that   if the default option for debit card shows when user enter his Credit card number it has to display card type automatically."
			)
	public void test() throws Throwable {
		try{
			...
			*/
			...
			logStep(stepInfo);
			PaymentPage.inputCreditCardNumber(Constants.MASTER_CARD_NUMBER);
			verifyStep(PaymentPage.verifyCardTypeIsMasterCard(),
					stepInfo);
